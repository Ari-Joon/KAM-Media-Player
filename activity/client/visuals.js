...
/**
 * Spikes: the spectrum wrapped into a radial burst.
 *
 * Mirrored across the vertical axis so the figure stays symmetrical, which is
 * what makes it read as an object rather than a chart bent into a circle.
 */
export class SpikesVisual extends Canvas2DVisual {
  render(score, playbackSec) {
    const { context, width, height } = this.begin(score, playbackSec);
    const lanes = this.lanes;
    const [from, to] = lanes.palette;
    ...
    const cx = width / 2;
    const cy = height / 2;
    const inner = Math.min(width, height) * (0.07 + lanes.bass * 0.05 + lanes.beat * 0.03);
    // Longer reach and a steeper response curve: the previous linear mapping
    // left even loud bands looking stubby.
    const reach = Math.min(width, height) * 0.46;
    const spin = this.lanes.scoreSec * (0.06 + lanes.flux * 0.10);

    context.lineCap = 'round';
    const total = bands * 2;
    for (let i = 0; i < total; i++) {
      // Mirror the second half so the burst is symmetrical.
      const band = i < bands ? i : total - 1 - i;
      const level = lanes.spectrum[band];
      // Emphasise the top of each band's range, so peaks read as spikes rather
      // than as a gently undulating disc.
      // A steeper curve: quiet bands stay short and loud ones shoot out, so the
      // silhouette is jagged rather than a gently undulating star.
      const shaped = Math.pow(level, 0.48);
      const angle = (i / total) * Math.PI * 2 + spin;
      const length = inner + shaped * reach;

      // Tapered: wide at the base, needle-thin at the tip. Thinner and longer
      // than before, which is what makes a spike read as a spike rather than a
      // wedge.
      const baseWidth = Math.max(1.5, Math.min(width, height) * 0.009);
      ...
      const nx = Math.sin(angle) * baseWidth / 2;
      const ny = -Math.cos(angle) * baseWidth / 2;

      context.fillStyle = mix(from, to, band / (bands - 1));
      context.globalAlpha = 0.35 + shaped * 0.65;
      context.beginPath();
      ...
      // A second, shorter spike offset between each pair, which doubles the
      // apparent density without doubling the cost of the main pass.
      if (i % 2 === 0) {
        const midAngle = angle + (Math.PI / total);
        const midLength = inner + shaped * reach * 0.55;
        context.globalAlpha = 0.20 + shaped * 0.35;
        context.beginPath();
        context.moveTo(cx + Math.cos(midAngle) * inner, cy + Math.sin(midAngle) * inner);
        context.lineTo(cx + Math.cos(midAngle) * midLength, cy + Math.sin(midAngle) * midLength);
        context.lineWidth = baseWidth * 0.6;
        context.strokeStyle = mix(from, to, band / (bands - 1));
        context.stroke();
      }

      // Bright tip on strong bands, which is what sells them as spikes.
      if (shaped > 0.45) {
        context.globalAlpha = (shaped - 0.45) * 1.6;
        context.fillStyle = '#ffffff';
        context.beginPath();
        context.arc(x1, y1, baseWidth * 0.30, 0, Math.PI * 2);
        context.fill();
      }
      context.globalAlpha = 1;
    }

    // Core that swells on the beat.
    const core = context.createRadialGradient(cx, cy, 0, cx, cy, inner * 1.5);
    core.addColorStop(0, `rgba(255,255,255,${0.25 + lanes.beat * 0.45})`);
